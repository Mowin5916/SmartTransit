...
export default function ConductorPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const [myProfile, hist, routesList, ass] = await Promise.all([
        getProfileById(userId),
        getWorkHistory(userId),
        getRoutes(),
        getAssignmentForUser(userId).catch(() => null)
      ]);
      ...
      if (ass) {
        ...
        if (ass.route_id) {
          const foundRoute = routesList.find(
            (r: any) => String(r.id) === String(ass.route_id)
          );

          setAssignedRoute(foundRoute || null);
          setSelectedRouteId(String(ass.route_id));
        } else {
    ...
    // Bonus: listen for assignment changes in real time
    const sub = supabase
  ...
  return (
    <div className="relative min-h-screen text-white p-4 md:p-6 font-sans">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        {/* HEADER */}
        <header className="max-w-4xl mx-auto bg-zinc-900/90 backdrop-blur p-4 rounded-2xl border border-white/10 mb-6 flex justify-between items-center shadow-2xl">
        ...
        <main className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-zinc-900/90 rounded-2xl shadow-xl border border-white/10 p-5">
  ...
  <div className="flex items-center justify-between bg-black/40 p-4 rounded-xl border border-white/10 mb-4">
    <div>
      <div className="text-xl font-bold text-white">
        {assignedRoute?.id || assignment.route_id}
        {assignedRoute?.name ? `: ${assignedRoute.name}` : ''}
      </div>
  ...
  <p className="text-sm text-zinc-500 italic mb-4 border-2 border-dashed border-white/10 rounded-xl p-4 text-center">
    No specific route assigned currently.
              ...
              <div className="border-t border-white/10 pt-4">
                ...
                {/* FIX: Use real bus id instead of DEMO */}
                {assignment?.bus_id ? (
                  <PassengerCounter busId={assignment.bus_id} />
                ) : (
            ...
            {/* EMERGENCY REPORTING */}
            <div className="bg-zinc-900/90 rounded-2xl shadow-2xl border border-red-500/20 overflow-hidden">
          ...
          {/* RIGHT: HISTORY */}
          <div className="bg-zinc-900/90 p-5 rounded-2xl shadow-xl border border-white/10 h-full">
            <h3 className="font-bold text-white mb-4">Shift History</h3>
